...
pub mod clearing_house {
    ...
    pub fn update_twaps(
        ...
        market_index: u64,
        new_mark_twap: u128,
        new_oracle_twap: i128,
    ) -> ProgramResult {

        let clock = Clock::get()?;
        let now = clock.unix_timestamp;

        let market = &mut ctx.accounts.markets.load_mut()?
            .markets[Markets::index_from_u64(market_index)];

        // update oracle prices 
        market.amm.last_oracle_price = new_oracle_twap;
        market.amm.last_oracle_price_twap = new_oracle_twap;
        market.amm.last_oracle_price_twap_ts = now;

        // update mark prices 
        market.amm.last_mark_price_twap = new_mark_twap;
